...
||Data sources that are simple download can be easily automated. Other data sources require logging in and might be trickier to automate.. Each source dataset may have a different access method (e.g. download, API), data schema, and update schedule| Latest OGL-licensed occupants are automatically exported from the Geographic Site Registry to a staging area using an FME script then verified manually.
