...
|Integrate|Integrate reference addresses (in PAX format) into reference road segments (blocks) and generate block ranges, access points, and address parcel ids. Reference addresses are rejected if:<br>* they don't conform to the PAX schema<br>* the given street doesn't exist in the given locality<br>* the given address location is too far from the given street<br>The address is a duplicate of an address that came from a higher-ranked data source. Sources are ranked as follows:<br>1. AddressBC<br>2. Open data munis<br>3. BC Assessment<br>|A standalone Java application (e.g., WAR file) called Geocodable BC Maker which contains an embedded instance of the geocoder.|
