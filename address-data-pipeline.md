...
|Transform|Transform each reference address dataset from its local schema and format to a single, standard schema and format called the [*Physical Address Exchange* (PAX) Standard](https://github.com/bcgov/ols-geocoder/blob/gh-pages/BCAddressExchangeSchema.md) PAX can support complex addressing scenarios more easily than the current target schema and format. For example, four-hundred unit-level addresses for an apartment building with 20 floors and 20 units per floor can be defined in two records, one for the apartment building itself and one for all the units using unitNumber expressions (e.g., 101-120,201-220,...2001-2020). |One ETL script (doesn't have to be FME) for each source data format (e.g., AddressBC, BC Assessment). We use FME for existing ETL scripts.
||Addresses are rejected if they fail field-level validation. Types of field validation failure include a field value not being the right type (e.g., a string where a number is expected), no value for a mandatory field, field value not in the allowed list of field values, and numeric field value out of range. Note that an address like 10 Main St W, Lanceville will pass field-level validation but the street may not actually exist in Lanceville. That existence check is done in the Integrate stage that follows. Rejected address are saved for QA analysis by the appropriate data authorities.
