...
||Addresses are rejected if they fail field-level validation. Types of field validation include a field value not of the right type (e.g., a string where a number is needed), no value for a mandatory field, field value not in allowed list of values, and numeric field value out of range. Note that an address like 10 Main St W, Lanceville will pass field-level validation but may not actually exist in Lanceville; that existence check is done in the Integrate stage that follows.
...
||All rejected addresses and occupants are saved for QA analysis by our data authorities|
