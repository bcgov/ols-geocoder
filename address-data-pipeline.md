...
|Integrate|Integrate reference addresses (in PAX format) into reference road segments (blocks) and generate block ranges, access points, and address parcel ids. When duplicates are encountered, addresses are ranked by their data source as follows:<br>1. AddressBC<br>2. Open data munis<br>3. BC Assessment<br>|A standalone Java application (e.g., WAR file) called Geocodable BC Maker which contains an embedded instance of the geocoder.|
