...
||Each source dataset may have a different access method (e.g. download, API), data schema, and update schedule. Data sources that require logging in might be trickier to automate.| Latest OGL-licensed occupants are automatically exported from the Geographic Site Registry to a staging area using an FME script then verified manually.
