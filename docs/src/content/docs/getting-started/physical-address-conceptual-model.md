---
title: Physical Address Conceptual Model
description: Defines what a physical address is and how it differs from a mailing address.
---
...
A civic address includes a civic number assigned by a local government to a site assigned to a street in a locality in a subCountry in a Country. A civic address may also include the name of a site and the names of its subsites. Here are some examples of civic addresses:
...
| Element Name | Data Type | Description | Required (Civic) | Required (Non-civic) |
|--------------|-----------|-------------|------------------|----------------------|
...
| fullSiteDescriptor | String | Full site descriptor starting with unit and SITE_NAME followed by all units and SITE_NAMEs in parent site hierarchy separated by commas | No | No |
...
| isStreetTypePrefix | Boolean | True if street type appears before street name | No | No |
| streetDirection | String | Official street direction abbreviation (e.g., N,S,E,W,NE,SE,NW,SW) | No | No |
| isStreetDirectionPrefix | Boolean | true if street direction appears before street name | No | No |
...
| isOfficial | Boolean | True if address is designated as official by the appropriate address authority | Yes | Yes |
| isNonCivic | Boolean | True if address has no assigned civic number | Yes | Yes |
...
- To determine the location of a site for display on a map
...
A subSite like a unit within a building or a townhouse within a complex can have its own site location and access point. If not specified, a subsite inherits its locations from its superSite. For example, if the location of individual units within a building are not known, it is sufficient to define the site location and access point of the building and all units within the building will inherit these locations. In the case of service or emergency entrances to a building, these can be represented as subsites of the building with each subSite having its own name, site location, and access point.
...
|--------------|-----------|-------------|
| siteLocation | Point | (x,y) coordinate location of a site; by default, coordinates are defined in lon,lat format |
