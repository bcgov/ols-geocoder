...
- Added a new preferredEncoding parameter that returns an ASCII alternative for a UTF-8 street name when one is provided by the address authority. Supported values are ascii, extended-ascii, and utf-8. The BC Address Geocoder response also includes an 'encoding' element which will list the encoding of the response address (fullAddress).
...
## January 22, 2025
- BC Address Geocoder version 4.5.1
